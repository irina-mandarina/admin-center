...
<template>
    <div class="relative inline-block">
        <button @click="opened = !opened" class="inline px-3 w-full py-1 text-left outline-1 outline-gray-500" :class="{
            'outline-2': opened,
            'outline-blue-400': opened
        }">
            {{ chosenOption }}
            <i v-if="!opened" class="fa fa-chevron-down float-right p-1"/>
            <i v-if="opened" class="fa fa-chevron-up float-right p-1"/>
        </button>
